...
void Game::playerChangeOutfit(uint32_t playerId, Outfit_t outfit) {
	...
	if (player->canWear(outfit.lookType, outfit.lookAddons)) {
		...
		// persist wings/aura/shader in a dedicated table so they survive relog
		{
			Database& db = Database::getInstance();
			db.executeQuery(fmt::format(
				"INSERT INTO `player_visuals` (`player_id`,`wings`,`aura`,`shader`) VALUES ({:d},{:d},{:d},{:d})"
				" ON DUPLICATE KEY UPDATE `wings`=VALUES(`wings`), `aura`=VALUES(`aura`), `shader`=VALUES(`shader`)",
				player->getGUID(), static_cast<uint32_t>(outfit.lookWings), static_cast<uint32_t>(outfit.lookAura), static_cast<uint32_t>(outfit.lookShaderId)));
		}
