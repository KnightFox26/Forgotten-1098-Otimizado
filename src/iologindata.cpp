...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result) {
    ...
    // load wings/aura/shader from dedicated table if present
    if (auto res2 = db.storeQuery(fmt::format("SELECT `wings`, `aura`, `shader` FROM `player_visuals` WHERE `player_id` = {:d}", player->getGUID()))) {
        player->defaultOutfit.lookWings = res2->getNumber<uint16_t>("wings");
        player->defaultOutfit.lookAura = res2->getNumber<uint16_t>("aura");
        player->defaultOutfit.lookShaderId = res2->getNumber<uint16_t>("shader");
    }
...
bool IOLoginData::savePlayer(Player* player) {
    ...
    // custom: save wings/aura/shader if columns exist (ignored by DB if not)
    ...
    query << "`lookshader` = " << static_cast<uint32_t>(player->defaultOutfit.lookShaderId) << ',';
