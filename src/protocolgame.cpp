...
// Wings/Auras catalog support (file-scope)
static std::vector<std::pair<uint16_t, std::string>> s_wingCatalog;
static std::vector<std::pair<uint16_t, std::string>> s_auraCatalog;
static bool s_wingsAurasLoaded = false;

// Shader catalog support (id->name)
static bool s_shadersLoaded = false;
static std::vector<std::pair<uint16_t, std::string>> s_shaderCatalog;

static void loadWingsAurasCatalog()
{
    if (s_wingsAurasLoaded) {
        return;
    }
    pugi::xml_document doc;
    pugi::xml_parse_result result = doc.load_file("data/XML/wings_auras.xml");
    if (!result) {
        printXMLError("ProtocolGame::loadWingsAurasCatalog", "data/XML/wings_auras.xml", result);
        s_wingsAurasLoaded = true; // avoid retry spam
        return;
    }

    pugi::xml_node root = doc.child("wingsauras");
    if (!root) {
        s_wingsAurasLoaded = true;
        return;
    }
    ...
    for (pugi::xml_node node : root.children("wing")) {
        pugi::xml_attribute idAttr = node.attribute("id");
        pugi::xml_attribute nameAttr = node.attribute("name");
        if (idAttr) {
            uint16_t id = pugi::cast<uint16_t>(idAttr.value());
            std::string name = nameAttr ? std::string{nameAttr.value()} : std::string{"Wing "} + std::to_string(id);
            s_wingCatalog.emplace_back(id, name);
        }
    }
    for (pugi::xml_node node : root.children("aura")) {
        pugi::xml_attribute idAttr = node.attribute("id");
        pugi::xml_attribute nameAttr = node.attribute("name");
        if (idAttr) {
            uint16_t id = pugi::cast<uint16_t>(idAttr.value());
            std::string name = nameAttr ? std::string{nameAttr.value()} : std::string{"Aura "} + std::to_string(id);
            s_auraCatalog.emplace_back(id, name);
        }
    }

    // Offsets are now handled client-side only

    s_wingsAurasLoaded = true;
}

static void loadShadersCatalog()
{
    if (s_shadersLoaded) {
        return;
    }
    ...
    pugi::xml_node root = doc.child("shaders");
    s_shaderCatalog.clear();
    for (pugi::xml_node node = root.first_child(); node; node = node.next_sibling()) {
        if (std::string(node.name()) == "shader") {
            uint16_t id = node.attribute("id").as_uint();
            std::string name = node.attribute("name").as_string();
            if (id != 0 && !name.empty()) {
                s_shaderCatalog.emplace_back(id, name);
            }
        }
    }
    s_shadersLoaded = true;
...
void ProtocolGame::parseSetOutfit(NetworkMessage& msg) {
    ...
    // OTCv8 client may send wings+aura after mount when feature is enabled
    if (player && player->getOperatingSystem() >= CLIENTOS_OTCLIENT_LINUX) {
        // try to read optional wings/aura (client sends them if GameWingsAndAura advertised)
        newOutfit.lookWings = msg.get<uint16_t>();
        newOutfit.lookAura = msg.get<uint16_t>();
        // optional shader name (string) if GameOutfitShaders advertised
        if (msg.getRemainingBufferLength() >= 2) { // at least string length
            std::string shaderName = msg.getString();
            if (!shaderName.empty()) {
                loadShadersCatalog();
                uint16_t shaderId = 0;
                for (const auto& it : s_shaderCatalog) {
                    if (it.second == shaderName) { shaderId = it.first; break; }
                }
                newOutfit.lookShaderId = shaderId;
            } else {
                newOutfit.lookShaderId = 0;
            }
        }
    }
	g_dispatcher.addTask([=, playerID = player->getID()]() {
...
void ProtocolGame::sendFeatures() {
    ...
    // Advertise OTCv8 features
    // Opcode matches Proto::GameServerFeatures (67 / 0x43) on the client
    NetworkMessage msg;
    msg.addByte(0x43);
    // number of feature flags below
    msg.add<uint16_t>(7);
    msg.addByte(30); msg.addByte(1); // GameChangeMapAwareRange
	msg.addByte(16); msg.addByte(1); // GameMagicEffectU16
    msg.addByte(104); msg.addByte(1); // GameWingsAndAura
    msg.addByte(114); msg.addByte(1); // GameWingOffset
    msg.addByte(106); msg.addByte(1); // GameOutfitShaders
    msg.addByte(115); msg.addByte(1); // AuraFrontAndBack
    msg.addByte(128); msg.addByte(1); // WingFrontAndBack (custom)
    writeToOutputBuffer(msg);
}
...
void ProtocolGame::sendOutfitWindow() {
    ...
    // Extend outfit window with wings/aura/shader lists for OTCv8
    loadWingsAurasCatalog();
    msg.addByte(std::min<size_t>(s_wingCatalog.size(), std::numeric_limits<uint8_t>::max()));
    for (const auto& it : s_wingCatalog) {
        msg.add<uint16_t>(it.first);
        msg.addString(it.second);
        // Offsets are now handled client-side only
    }
    msg.addByte(std::min<size_t>(s_auraCatalog.size(), std::numeric_limits<uint8_t>::max()));
    for (const auto& it : s_auraCatalog) {
        msg.add<uint16_t>(it.first);
        msg.addString(it.second);
        // Offsets are now handled client-side only
    }
    // shaders list (id + name) if GameOutfitShaders
    loadShadersCatalog();
    msg.addByte(std::min<size_t>(s_shaderCatalog.size(), std::numeric_limits<uint8_t>::max()));
    for (const auto& it : s_shaderCatalog) {
        msg.add<uint16_t>(it.first);
        msg.addString(it.second);
    }
...
void ProtocolGame::AddOutfit(NetworkMessage& msg, const Outfit_t& outfit) {
    ...
    // OTCv8 extensions: wings and aura (u16 each)
    msg.add<uint16_t>(outfit.lookWings);
    msg.add<uint16_t>(outfit.lookAura);
    // OTCv8 extension: shader name (string) when GameOutfitShaders is advertised
    loadShadersCatalog();
    std::string shaderName;
    for (const auto& it : s_shaderCatalog) {
        if (it.first == outfit.lookShaderId) { shaderName = it.second; break; }
    }
