...
struct Outfit_t {
    ...
    // OTCv8 extensions: treat wings as a second aura channel client-side
    uint16_t lookWings = 0;
    uint16_t lookAura = 0;
	// OTCv8 extension: shader id (mapped to client shader name via data/XML/shaders.xml)
	uint16_t lookShaderId = 0;
	uint8_t lookHead = 0;
	uint8_t lookBody = 0;
	uint8_t lookLegs = 0;
	uint8_t lookFeet = 0;
